...
fn pop_jump_push_unsafe(
    num_nodes: usize,
    sequence_indices: &mut [usize],
    jump_indices: &[usize],
    labels: &[usize],
    output: u8,
) {
    ...
    unsafe {
        while i > 0 {
            visit(&*ptr::slice_from_raw_parts(raw_ptr, i), labels, output);
            i -= 1;
            j = *jump_indices.get_unchecked(*sequence_indices.get_unchecked(i));
            while j < num_nodes {
                *sequence_indices.get_unchecked_mut(i) = j;
                i += 1;
                j += 1;
            }
        }
...
fn pop_jump_push(
    ...
    assert_eq!(num_nodes, sequence_indices.len());
    assert_eq!(num_nodes, jump_indices.len());
    assert_eq!(num_nodes, labels.len());
    assert!(jump_indices.iter().all(|x| x <= &num_nodes));
...
pub(crate) fn pop_jump_push_unsafe_main(
    root: usize,
    parents: &[usize],
    children: &Vec<usize>,
    output: u8,
